...
public:
  ...
  //fatboy(const fatboy& boy)
  //{}

  //fatboy(fatboy&& boy) = default;
  /*
  {
    std::swap(_fat_data,boy._fat_data);
    _value = boy._value;
  }
  */
private:
...
class mother {
public:
  mother() : _data(1000,fatboy(10))
  {}
  ~mother(){}

  void reset()
  { _data.clear(); _data.resize(1000,fatboy(0)); }
...
public:
  ...
  void swap_child(std::vector<fatboy>&& kid_v)
  { std::swap(_hidden_kid,kid_v); }
...
int main() {
  ...
  my_watch.Start();
  dad.swap_child(std::move(mom._data));
